/**
 * RESTO NUSANTARA PELANGGAN - DARK STORE
 * Database bersama menggunakan localStorage + BroadcastChannel
 */
...
const INITIAL_MENU = [
    {
        ...
        kategori: 'Makanan',
        harga: 28000,
        deskripsi: 'Mie lembut dengan topping ayam cincang, bakso, dan pangsit goreng. Kuah kaldu segar.',
    ...
    },
    {
        ...
        kategori: 'Makanan',
        harga: 45000,
        deskripsi: 'Ayam bakar bumbu rempah khas Nusantara dengan sambal terasi.',
    ...
    },
    {
        ...
        kategori: 'Minuman',
        harga: 8000,
        deskripsi: 'Teh pilihan dengan gula asli, disajikan dingin dengan es batu.',
...
Store.getStats = function() {
    ...
    return {
        ...
        pesananHariIni: todayOrders.length,
        selesaiHariIni: todayOrders.filter(o => o.status === 'Selesai').length,
        totalPenjualan: todayOrders
            .filter(o => o.pembayaranStatus === 'Lunas' || o.status === 'Selesai')
            .reduce((sum, o) => sum + (o.total || 0), 0),
        menunggu: orders.filter(o => o.status === 'Baru').length,
...
// ==================== REAL-TIME LISTENER ====================
Store.onOrdersChange = function(callback) {
    // Dengar perubahan dari tab lain
    window.addEventListener('storage', (e) => {
        if (e.key === STORE_KEYS.ORDERS || e.key === 'rn_sync_trigger') {
            callback();
        }
    });
    
    // Dengar BroadcastChannel
    if (broadcastChannel) {
        broadcastChannel.onmessage = (event) => {
            if (event.data.type === 'NEW_ORDER' || 
                event.data.type === 'ORDER_STATUS_CHANGED' ||
                event.data.type === 'ORDER_PAYMENT_CHANGED') {
                callback();
            }
        };
    }
